...
/* Controllers */
app.controller('RootController', ['$scope', 'ServiceUtiles', 'ServiceHistorique', '$timeout','$filter', function ($scope, ServiceUtiles, ServiceHistorique, $timeout, $filter) {
            ...
            ServiceHistorique.getAllNoneSeenHistorique(idSociete).then(function (data) {
                ...
                for (var i = 0; i < $scope.Historiques.length; i++) {
                    ...
                    if ($scope.Historiques[i].typeAction !== 16 && $scope.Historiques[i].typeAction !== 17 && $scope.Historiques[i].typeAction !== 19 && $scope.Historiques[i].typeAction !== 31) {
                        ...
                        user = $scope.affichageStringUser($scope.Historiques[i].idUser, $scope.app.user.idUser,username);
                    } else {
                        user = "";
                    }
        ...
        $scope.dateToday = function (dateTest) {
            ...
            var DateToday = date.getDate() + "-" + (date.getMonth() + 1) + "-" + date.getFullYear(); //+ " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();
            var DateYesterday = (date.getDate() - 1) + "-" + (date.getMonth() + 1) + "-" + date.getFullYear(); //+ " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();
