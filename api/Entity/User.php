...
class User extends API {
                ...
                if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    try {
                        ...
                        if ($sql_user->rowCount() > 0) {
                            ...
                            $sql_soc = $this->db->prepare("SELECT idSociete, nomSoc, idabonnement FROM societe WHERE idSociete = :idSociete LIMIT 1");
        ...
        if (isset($_REQUEST['data'])) {
            ...
            if (!empty($email) and ! empty($password)) {
                if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    try {
                        $sql_checkSoc = $this->db->prepare("SELECT nomSoc, email FROM societe WHERE nomSoc = :nom OR email = :email ");
                        $sql_checkSoc->bindParam('nom', $nom, PDO::PARAM_STR);
                        $sql_checkSoc->bindParam('email', $email, PDO::PARAM_STR);
                        $sql_checkSoc->execute();

                        if ($sql_checkSoc->rowCount() <= 0) {

                            $sql_soc = $this->db->prepare("INSERT INTO societe (nomSoc, email, tel, dateCreation, idabonnement)"
                                    . "VALUES(:nom, :email, :tel, :dateCreation, :typeAbonnement)");
                            ...
                            if ($sql_soc) {
                                $sql_get_soc = $this->db->prepare("SELECT idSociete,nomSoc FROM societe WHERE email = :email AND dateCreation = :dateCreation");
    ...
    public function UpdateSubUser() {
        ...
        if (isset($_REQUEST['data'])) {
            ...
            try {
                $sql_user = $this->db->prepare("UPDATE user SET nom = :nom, prenom = :prenom, tel = :tel WHERE idUser = :idUser");
    ...
    public function UpdateUser() {
        ...
        if (isset($_REQUEST['data'])) {
            ...
            if (!empty($idUser)) {
                ...
                try {
                    $sql = $this->db->prepare("UPDATE user SET nom = :nom, prenom = :prenom, tel = :tel, photo = :photo "
                            . "WHERE idUser = :idUser");
