...
class Utiles extends API {
    ...
    public function GetRevenuPaiementOnFAByYear() {
        ...
        if (isset($_REQUEST['data'])) {
            ...
            if (!empty($idSociete) && !empty($filterYear)) {
                try {
                    $sql = $this->db->prepare("select fa.idfacture as idFactureAchat, ifnull(SUM(p.montant),0) as totalPAchat FROM factureachat fa INNER JOIN paiement p ON fa.idFacture = p.idFacture WHERE fa.idSociete = :idSociete AND YEAR(p.datePaiement) = :filterYear");
                    $sql->bindParam('idSociete', $idSociete, PDO::PARAM_INT);
                    $sql->bindParam('filterYear', $filterYear, PDO::PARAM_STR);
                    $sql->execute();
    ...
    public function GetRevenuPaiementOnFVByYear() {
        ...
        if (isset($_REQUEST['data'])) {
            ...
            if (!empty($idSociete) && !empty($filterYear)) {
                try {
                    $sql = $this->db->prepare("select fv.idfacture as idFactureVente, ifnull(SUM(p.montant),0) as totalPvente FROM facturevente fv INNER JOIN paiement p ON fv.idFacture = p.idFacture WHERE fv.idSociete = :idSociete AND YEAR(p.datePaiement) = :filterYear");
                    $sql->bindParam('idSociete', $idSociete, PDO::PARAM_INT);
                    $sql->bindParam('filterYear', $filterYear, PDO::PARAM_STR);
                    $sql->execute();
